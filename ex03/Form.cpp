...
{
    std::cout << "AForm " << _name << " created. The grade required to Sign the form is " << _gradeRequiredToSign << ". The grade required to execute the form is " << _gradeRequiredToExecute << " The form status is " << getFormStatus() << std::endl;
}
...
const std::string AForm::getFormStatus(void) { return (_isFormSigned == 0 ? "not Signed" : "Signed"); }
...
void AForm::execute(Bureaucrat const & executor) const
{
    if (this->_isFormSigned == true
            && executor.getGrade() <= this->getGradeRequiredToExecute())
        {
            action();
        }
        else if (this->_isFormSigned == false)
